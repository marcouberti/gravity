	...
	public static void updateUniverse() {
		...
		*/
		...
		for(SpaceObject so: spaceObjects) {
			...
			//LIMITI UNIVERSO
			if(so.x+so.radius > ApplicationManager.SCREEN_H) {
				so.x = ApplicationManager.SCREEN_H-so.radius;
				so.velx = -so.velx;
				//so.destroyed = true;
			}
			
			if(so.y+so.radius > ApplicationManager.SCREEN_W){
				so.y = ApplicationManager.SCREEN_W-so.radius;
				so.vely = -so.vely;
				//so.destroyed = true;
			}
